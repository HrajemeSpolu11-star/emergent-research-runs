# Session sess:publish-endpoint-real-selftest
...
Agent `agent:endpoint-selftest` byl testován 0.0 minut, 8 kroků.
Zaznamenáno 297 telemetrických eventů.

## Paměť: co se změnilo

- Episodická paměť: 0 → 8 záznamů
  (8 nových, 0 odstraněných/konsolidovaných,
  0 změněných).
...
- `interact` byl navržen, ale zamítnut arbitráží 8×
- `wait` byl navržen, ale zamítnut arbitráží 8×
...
`SubconsciousGuardian` aktivně zasáhl 8×.
